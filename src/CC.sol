...
contract CC is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, OwnableUpgradeable{
    string private constant NAME = "CC";
    string private constant SYMBOL = "CC";
    uint256 public constant MIN_MINT_INTERVAL = 365 days;
    uint256 public constant MINT_CAP_DENOMINATOR = 10_1000;
    ...
    error CCToken_ImproperlyInitialized();
    error CCToken_MintAmountTooLarge(uint256 amount, uint256 maximumAmount);
    error CCToken_NextMintTimestampNotElapsed(uint256 currentTimestamp, uint256 nextMintTimestamp);
    error CCToken_MintCapNumeratorTooLarge(uint256 numerator, uint256 maximumNumerator);
    ...
    function initialize(uint256 _initialSupply, address _owner) public initializer {
        if (_initialSupply == 0 || _owner == address(0)) revert CCToken_ImproperlyInitialized();
    ...
    function mint(address _recipient, uint256 _amount) public onlyOwner {
        ...
        if (_amount > maximumMintAmount) {
            revert CCToken_MintAmountTooLarge(_amount, maximumMintAmount);
        }
        if (block.timestamp < nextMint) revert CCToken_NextMintTimestampNotElapsed(block.timestamp, nextMint);
    ...
    function setMintCapNumerator(uint256 _mintCapNumerator) public onlyOwner {
        if (_mintCapNumerator > MINT_CAP_MAX_NUMERATOR) {
            revert CCToken_MintCapNumeratorTooLarge(_mintCapNumerator, MINT_CAP_MAX_NUMERATOR);
        }
